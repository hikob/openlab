...
void rcc_sysclk_select_source(rcc_sysclk_source_t source)
{
    ...
    *rcc_get_CFGR() = reg;
    ...
    while (((*rcc_get_CFGR() >> 2) & RCC_CFGR__SW_MASK) != source)
    {
    }
}
...
void rcc_sysclk_set(rcc_sysclk_source_t source,
                    ...
                    rcc_sysclk_apb_prescaler_t apb2_pres)
{
    uint32_t cfgr = *rcc_get_CFGR() & 0xFFFF0000;
    *rcc_get_CFGR() = cfgr | source | apb2_pres << 11 | apb1_pres << 8
                      | ahb_pres << 4;
}
...
uint32_t rcc_sysclk_get_clock_frequency(rcc_sysclk_clock_t clock)
{
    ...
    switch (*rcc_get_CFGR() & RCC_CFGR__SW_MASK)
    {
        ...
        case RCC_SYSCLK_SOURCE_HSI:
            ...
            break;
        case RCC_SYSCLK_SOURCE_MSI:
            // SYSCLK is MSI current speed
            clk = rcc_msi_get_frequency();
            break;
    ...
    {
        // In theory, the SYSTICK clock should be HCLK/8
        return clk;
    }
...
{
    // Disable output
    *rcc_get_CFGR() &= ~(RCC_CFGR__MCOSEL_MASK | RCC_CFGR__MCOPRE_MASK);

    // Set selected clock and prescaler
    *rcc_get_CFGR() |= (clock << 24) | prescaler << 28;
}
