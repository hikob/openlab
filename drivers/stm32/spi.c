                                ...
                                uint8_t *rx_buffer, uint16_t length)
{
    ...
    // Start the RX DMA, with transfer done handler
    dma_start(_spi->dma_channel_rx, (handler_t) transfer_done, _spi);

    // Start the TX DMA, with no handler
    dma_start(_spi->dma_channel_tx, NULL, NULL);
