...
{
    {
        ...
        .bEndpointAddress   = USB_ENDPOINT1_IN,
...
static const uint16_t hid_langid_string[]  = {0x0409};
static const uint16_t hid_vendor_string[]  = {'H', 'i', 'K', 'o', 'B'};
static const uint16_t hid_product_string[] = {'A', 'g', 'i', 'l', 'e', ' ', 'F', 'o', 'x'};
static uint16_t hid_serial_string[48];

static const usb_string_desc_t hid_string_desc[] =
{
    // Lang ID
    {
        .bLength = 2 + sizeof(hid_langid_string),
        .bDescriptorType = USB_DESC_STRING,
        .wData = hid_langid_string
    },
    // Manufacturer
    {
        .bLength = 2 + sizeof(hid_vendor_string),
        .bDescriptorType = USB_DESC_STRING,
        .wData = hid_vendor_string
    },
    // Product
    {
        .bLength = 2 + sizeof(hid_product_string),
        .bDescriptorType = USB_DESC_STRING,
        .wData = hid_product_string
    },
    // Serial
    {
        .bLength = 2 + sizeof(hid_serial_string),
        .bDescriptorType = USB_DESC_STRING,
        .wData = hid_serial_string
    }
};
...
const usb_profile_t hid =
{
    .device_descriptor = &hid_dev_desc,

    .device_qualifier_descriptor = &hid_dev_qual_desc,

    .number_of_string_descriptors = sizeof(hid_string_desc) / sizeof(usb_string_desc_t),
    .string_descriptors = hid_string_desc,
...
static void hid_standard_interface(usb_device_request_t *req)
{
    switch(req->bRequest)
    {
        ...
        default:
            log_warning("Standard interface not handled: bRequest = 0x%02X bmRequestType = 0x%02X wValue = 0x%04X wIndex = 0x%04X wLength = 0x%04X", req->bRequest, req->bmRequestType, req->wValue, req->wIndex, req->wLength);
    }
}
