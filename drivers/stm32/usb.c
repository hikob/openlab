...
static uint16_t max_packet_size[8] = {0, 0, 0, 0, 0, 0, 0, 0};
...
static usb_endpoint_callback_t endpoint_callback[7];
static const usb_profile_t *profile;
static usb_state_t state;
static uint8_t usb_address;
static usb_data_callback_t data_callback[8];

void usb_init(const usb_profile_t *p)
{
    uint16_t i;
    ...
    *usb_get_CNTR() = USB_CNTR__CTRM | USB_CNTR__PMAOVRM | USB_CNTR__ERRM | USB_CNTR__WKUPM | USB_CNTR__SUSPM | USB_CNTR__RESETM;
...
static void usb_write_pma(uint8_t addr, const uint8_t *buf, uint16_t len)
{
    ...
    for(i = 0; i < len; i += 2)
    {
        *packet_memory = *((const uint16_t *)(&buf[i]));
        packet_memory += 2;
    }
}
...
void usb_send(uint8_t endp, bool first_time, usb_data_token_t first_token, const uint8_t *buf, uint16_t len)
{
    ...
    if(descriptor_len[endp] > max_packet_size[endp])
    {
        usb_write_packet(endp, descriptor_buf[endp], max_packet_size[endp]);
        descriptor_buf[endp] += max_packet_size[endp];
        descriptor_len[endp] -= max_packet_size[endp];
...
static void usb_endpoint_init(uint8_t endp, usb_endp_type_t type, uint16_t len, usb_endpoint_callback_t callback)
{
    ...
    log_info("Endpoint init: 0x%02x (type: %d), len: %d, callback: 0x%X", endp, type, len, callback);
...
{
    local_buf[0] = string_descriptor->bLength;
    local_buf[1] = string_descriptor->bDescriptorType;
...
static void fill_buf_configuration(const usb_conf_desc_t *configuration_descriptor)
{
    ...
    usb_conf_desc_t *conf = (usb_conf_desc_t *)local_buf;
...
static void register_enpoint_callbacks()
{
    ...
    for(i = 0; i < profile->device_descriptor->bNumConfigurations; i++)
    {
        configuration = &(profile->device_descriptor->configuration_descriptors[i]);
...
{
    log_info("Select configuration: %d", index);
    usb_configuration = index;
}

bool address_received(uint8_t endp)
{
    log_info("Setting device address: %d", usb_address);
...
{
    // This function is called quite often, the following log is quite verbose !
...
static void process_standard_device(usb_device_request_t *header)
{
    ...
    switch(header->bRequest)
    {
        ...
        case 0x01: // CLEAR_FEATURE
            log_error("CLEAR_FEATURE");
            break;

        case 0x03: // SET_FEATURE
            log_error("SET_FEATURE");
            break;
        ...
        case 0x06: // GET_DESCRIPTOR
            ...
            switch(header->wValue >> 8)
            {
                case 0x01: // DEVICE_DESCRIPTOR
                    ...
                    if(profile->device_descriptor->class_specific_len == 0)
                    {
                        usb_send(0, true, DATA1, (const uint8_t *)(profile->device_descriptor), header->wLength);
                    }
                    else
                    {
                        // If there is class or vendor specific data, append them after the descriptor
                        fill_buf((const uint8_t *)(profile->device_descriptor), 0, USB_DEV_DESC_SIZE);
                        fill_buf(profile->device_descriptor->class_specific, USB_DEV_DESC_SIZE, profile->device_descriptor->class_specific_len);
                        usb_send(0, true, DATA1, local_buf, USB_DEV_DESC_SIZE + profile->device_descriptor->class_specific_len);
                    }
                ...
                case 0x02: // CONFIG_DESCRIPTOR
                    ...
                    if(idx < profile->device_descriptor->bNumConfigurations)
                    {
                        fill_buf_configuration(&(profile->device_descriptor->configuration_descriptors[idx]));
                        usb_send(0, true, DATA1, local_buf, header->wLength);
                    }
                    else
                    {
                ...
                case 0x03: // STRING_DESCRIPTOR
                    ...
                    if(idx < profile->number_of_string_descriptors)
                    {
                        fill_buf_string(&(profile->string_descriptors[idx]));
                        usb_send(0, true, DATA1, local_buf, header->wLength);
                    }
                    else
                    {
                ...
                case 0x06: // DEVICE_QUALIFIER_DESCRIPTOR
                    ...
                    usb_send(0, true, DATA1, (const uint8_t *)(profile->device_qualifier_descriptor), USB_DEV_QUAL_DESC_SIZE);
                    break;
        ...
        case 0x09: // SET_CONFIGURATION
            ...
            switch(state)
            {
                case USB_STATE_ADDRESS:
                    ...
                    if(header->wValue != 0)
                    {
                        if(header->wValue <= profile->device_descriptor->bNumConfigurations)
                        {
...
static void endpoint0_callback(endpoint0_callback_t type)
{
    ...
    switch(type)
    {
        case ENDP0_IN:
        case ENDP0_OUT:
            log_debug("%s", type == ENDP0_IN ? "ENDP0_IN" : "ENDP0_OUT");
        ...
        case ENDP0_SETUP:
            ...
            if(len == 8)
            {
                ...
                log_debug("wLength: %d bRequest: 0x%02X bmRequestType: 0x%02X wValue: 0x%04x wIndex: 0x%04x",
                ...
                switch(setup_header.bmRequestType & 0x7F)
                {
                    ...
                    case 0x01: // Standard interface requests
                        ...
                        if(profile->standard_interface)
                        {
                            profile->standard_interface(&setup_header);
                        }
                        else
                        {
                    ...
                    case 0x02: // Standard endpoint requests
                        ...
                        if(profile->standard_endpoint)
                        {
                            profile->standard_endpoint(&setup_header);
                        }
                        else
                        {
                    ...
                    case 0x20: // Class device requests
                        ...
                        if(profile->class_device)
                        {
                            profile->class_device(&setup_header);
                        }
                        else
                        {
                    ...
                    case 0x21: // Class interface requests
                        ...
                        if(profile->class_interface)
                        {
                            profile->class_interface(&setup_header);
                        }
                        else
                        {
                    ...
                    case 0x22: // Class endpoint requests
                        ...
                        if(profile->class_endpoint)
                        {
                            profile->class_endpoint(&setup_header);
                        }
                        else
                        {
                    ...
                    case 0x40: // Vendor device requests
                        ...
                        if(profile->vendor_device)
                        {
                            profile->vendor_device(&setup_header);
                        }
                        else
                        {
                    ...
                    case 0x41: // Vendor interface requests
                        ...
                        if(profile->vendor_interface)
                        {
                            profile->vendor_interface(&setup_header);
                        }
                        else
                        {
                    ...
                    case 0x42: // Vendor endpoint requests
                        ...
                        if(profile->vendor_endpoint)
                        {
                            profile->vendor_endpoint(&setup_header);
                        }
                        else
                        {
            ...
            {
                log_error("Wrong setup header size: %d\r\n\tData content:", len);
                usb_read_pma(0, local_buf, len);

                for(i = 0; i < len; i++)
                {
                    printf("0x%02X ", local_buf[i]);
                }
...
void usb_lp_can1_rx0_isr()
{
    ...
    if(istr & USB_ISTR__RESET)
    {
        ...
        *usb_get_DADDR() = USB_DADDR__EF;
        ...
        usb_endpoint_init(0, USB_ENDPOINT_CONTROL, profile->device_descriptor->bMaxPacketSize0, (usb_endpoint_callback_t)0);
    ...
    {
        // We do not support suspend mode so we as to resume
        resume_cpt = 2;
        resume_state = 1;
        // Acknowledge interrupt
